...
export default function Certificate(props) {
  ...
  const takeNotes = async (idDoc) => {
    const docRef = doc(db, "usuarios", idUsuario, "nota", idDoc);
    const docSnap = await getDoc(docRef);

    if (docSnap.exists()) {
      avaliates.push(docSnap.data().total);
